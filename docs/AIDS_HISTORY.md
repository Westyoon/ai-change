...
> 이 문서는 AIDS 기능·밸런스 브랜치의 원래 작업 기록입니다. `dev` 통합 과정에서 확인된 경로·입력 계약·실제 밸런스 차이는 문서 마지막의 “통합 보정”과 루트 [History.md](../History.md)를 기준으로 합니다.
...
## 2026-08-25 (dev 통합 보정)

- 최신 스캐폴딩의 학과 코드 폴더 규칙에 맞춰 런타임 경로를 `js/minigames/AIDS/`로 확정했습니다.
- 실제 공통 입력 계약은 `MOVE_LEFT`/`MOVE_RIGHT`이므로 기존 기록의 `SELECT_LEFT`/`SELECT_RIGHT` 설명을 바로잡았습니다. 입력 모듈은 정적 import이며, 선택적인 부분은 host의 `context.input` 주입입니다.
- module 기반 harness는 `js/minigames/AIDS/dev/`로 이동했습니다. 오래된 물리 구현과 대화 transcript가 섞인 `aids-lite.html`은 중복·오염 파일로 판단해 제거했습니다.
- 현재 런타임 spawn 간격은 최신 balance 커밋 기준 4.5초 → 3.5초 → 2.5초입니다. D-09가 아직 TBD이므로 확정값이 아니라 통합 프로토타입 수치로 기록합니다.
- 공통 lifecycle 계약 테스트에서 abort, pause/resume, stale attempt 차단, 시도별 1회 완료, restart, 중복 destroy를 검증했습니다.

## 2026-09-10 (좌우 판정·경사 낙하 보정)

- 알의 중심이 필드의 왼쪽 또는 오른쪽 경계를 지나면 상자 DOM과 실제로 겹치지 않아도 각각 `인지` 또는 `데사` 방향으로 판정합니다. 따라서 세 번째 줄 발판에서 바깥쪽으로 빠진 알도 더 이상 자동 `이탈` 처리되지 않습니다.
- 알의 좌표가 중심점이라는 계약에 맞춰 착지·굴림·리사이즈의 높이 계산을 하나로 통일했습니다. 착지 다음 프레임에 알이 반지름만큼 발판 안으로 내려가던 현상을 제거했습니다.
- 발판 끝에서 떨어질 때 경사 접선의 하향 속도를 이어받고, 발판을 빗나간 경우에도 기존 낙하 속도를 유지하도록 수정했습니다.
- 보이는 발판과 물리 경사가 서로 다른 시간에 바뀌지 않도록 발판 방향 전환을 즉시 반영합니다.
- 낙하 유도는 다음 발판의 중앙 한 점을 계속 추적하지 않고, 예상 착지점이 발판의 안전 범위를 벗어날 때만 가장 가까운 범위까지 보정합니다. 마지막 분류 구간에서는 좌우 관성을 그대로 유지합니다.
- 좌우 경계 판정, 바깥쪽 경로, 경사면 높이 연속성, 리사이즈 후 높이, 자유 낙하 관성을 회귀 테스트로 고정했습니다.
