...
AI는 원본 `480×640`, CS는 `480×480` Canvas, CSE는 `440×920`, AIDS는 `390×740` 논리 좌표를 유지한 채 표시 크기만 화면에 맞춰 확대·축소합니다. CS의 Canvas와 조작 패널은 공간이 충분하면 원본 비율대로 나란히 커지고, 부족하면 원래 prototype처럼 위아래로 줄바꿈됩니다. CSE의 긴 세로 화면은 내부를 찌그러뜨리지 않고 스크롤해서 확인합니다. AIDS 발판의 보이는 면과 공의 착지·굴림 판정은 같은 `platformHalfLen`에서 계산되며 전체 프레임 배율을 함께 적용받습니다.
