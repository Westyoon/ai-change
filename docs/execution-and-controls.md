...
현재 Story 맵은 자유 이동 구현 전이므로 학과 카드를 클릭·터치하거나 키보드로 선택합니다. 카드를 선택하면 학과 이야기와 게임 안내를 차례로 본 뒤 미니게임이 시작됩니다. 사후게임은 `stat-boss`, `data-sphinx`, `control-boss`, `xr-egg-trials` 4종이 published 상태이며, 서로 다른 학과 미니게임 5종을 모두 완료하면 배틀 메뉴에서 실행할 수 있습니다. 교체된 전역 prototype과 개발 harness는 production `dist/`에 포함하지 않습니다.
